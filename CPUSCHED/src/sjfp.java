...
public class sjfp implements ActionListener{
	...
	public int [] arrivalTime, burstTime, ct, ta, wt, f, k;	
	int temp;
  	...
  	public void computeNonPre(){
        ...
        int size = bt.size();	
      	...
      	while(true){
	    	...
	    	if(c != last_c && last_c > -1 && last_c != size){
                if(gantt.isEmpty()){
	    			gantt.add(st);
	    			pid.add(c);
	    ...
	    int r = 0,g = 0,b = 0;
		    ...
		    for(int i=1; i<gantt.size(); i++){
		        ...
		        Ps[i] = new JLabel("P"  + pid.get(i)  +" (" + gantt.get(i) +")");
    ...
    public static void main(String[] args){new sjfp();}
}
